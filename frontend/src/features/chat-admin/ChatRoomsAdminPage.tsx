...
export function ChatRoomsAdminPage() {
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["chat", "admin", "rooms"] }),
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {t("chat_admin.subtitle", { defaultValue: "Create cohort/advisory rooms and archive when done." })}
          </p>
        </div>
        <Button
          variant="outline"
          size="sm"
          onClick={() => qc.invalidateQueries({ queryKey: ["chat", "admin", "rooms"] })}
          disabled={isFetching}
        >
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 sm:grid-cols-[1fr,200px,auto]">
          <div className="space-y-1.5">
            <Label>{t("chat_admin.room_name", { defaultValue: "Room name" })}</Label>
            <Input
              placeholder={t("chat_admin.room_placeholder", { defaultValue: "PhD 2025 • Public Health" })}
              value={form.name}
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={form.type}
              onValueChange={(v) => setForm((prev) => ({ ...prev, type: v as RoomForm["type"] }))}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                ...
                <SelectItem value="advisory">{t("chat.types.advisory")}</SelectItem>
          ...
          <div className="flex items-end">
            ...
            >
              ...
              {createMutation.isPending ? t("common.loading") : t("chat_admin.create")}
            </Button>
          </div>
        </CardContent>
      </Card>
              ...
              <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      {t("chat_admin.manage_members", { defaultValue: "Members" })}
                    </Button>
                    <Button
                      ...
                      className="flex items-center gap-2"
                    >
                      {room.is_archived ? <Check className="h-4 w-4" /> : <Archive className="h-4 w-4" />}
                      {room.is_archived
                        ? t("chat_admin.unarchive", { defaultValue: "Unarchive" })
      ...
      <Modal open={!!manageRoomId} onOpenChange={(open) => !open && setManageRoomId(null)} size="lg">
        <div className="space-y-3">
              ...
              <div className="border rounded-md divide-y max-h-48 overflow-y-auto">
                {searchQuery.isFetching ? (
                  ...
                  <div className="p-3 text-sm text-muted-foreground">
                    {t("chat_admin.no_users", { defaultValue: "No users found" })}
                  </div>
                ) : (
                  searchQuery.data?.map((u) => (
                    <div key={u.id} className="flex items-center justify-between px-3 py-2">
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => addMemberMutation.mutate({ user_id: u.id })}
          ...
          <div className="border rounded-md">
            <div className="px-3 py-2 text-sm font-semibold">
              {t("chat_admin.current_members", { defaultValue: "Current members" })}
            </div>
            <div className="divide-y max-h-64 overflow-y-auto">
              {membersQuery.isLoading ? (
                <div className="p-3 text-sm text-muted-foreground">{t("chat.loading_rooms")}</div>
              ) : (membersQuery.data ?? []).length === 0 ? (
                <div className="p-3 text-sm text-muted-foreground">
                  {t("chat_admin.no_members", { defaultValue: "No members yet" })}
                </div>
              ) : (
