...
export default function RP2HearingPrepScene({
  ...
  const nextOnComplete = (Array.isArray(node.next) ? node.next[0] : undefined) || node.outcomes?.[0]?.next?.[0];
      ...
      <div className="lg:col-span-3 min-h-0 overflow-auto space-y-4">
        ...
        {readOnly && (
          <div className="mt-3 text-sm text-muted-foreground whitespace-pre-line">
            {t(
              {
                ru: `Если подготовка к заслушиванию завершена${submittedAt ? ` (дата: ${new Date(submittedAt).toLocaleDateString()})` : ""}.`,
                kz: `Егер тыңдауға дайындық аяқталған болса${submittedAt ? ` (күні: ${new Date(submittedAt).toLocaleDateString()})` : ""}.`,
                en: `If the hearing preparation is completed${submittedAt ? ` (date: ${new Date(submittedAt).toLocaleDateString()})` : ""}.`,
              },
              ""
            )}
          </div>
