      ...
      <div className="lg:col-span-3 min-h-0 overflow-auto space-y-4">
        ...
        {readOnly && (
          <div className="mt-3 text-sm text-muted-foreground whitespace-pre-line">
            {t(
              {
                ru: `Если аттестационное дело сформировано${submittedAt ? ` (дата: ${new Date(submittedAt).toLocaleDateString()})` : ""}. ${guardMessage}`,
                kz: `Егер аттестаттау ісі қалыптастырылған болса${submittedAt ? ` (күні: ${new Date(submittedAt).toLocaleDateString()})` : ""}. ${guardMessage}`,
                en: `If the attestation file is complete${submittedAt ? ` (date: ${new Date(submittedAt).toLocaleDateString()})` : ""}. ${guardMessage}`,
              },
              ""
            )}
          </div>
