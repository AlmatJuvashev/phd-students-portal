...
const TYPE_STYLES: Record<EventType, string> = {
  ...
  meeting: 'bg-amber-50 border-amber-200 text-amber-700', // Added missing type style
  deadline: 'bg-orange-50 border-orange-200 text-orange-700', // Added missing type style
};
    ...
    <div ref={scrollRef} className="flex flex-col h-full bg-white rounded-xl shadow-sm border border-slate-200 overflow-y-auto custom-scrollbar p-3 sm:p-4 space-y-4">
                 ...
                 dayEvents.map(event => (
                   <button
                     ...
                     className={cn(
                       ...
                       TYPE_STYLES[event.type as EventType] || TYPE_STYLES['academic']
                     )}
                   >
                   ...
                   isCurrentDay ? (
                       <div className="text-sm text-slate-400 italic py-2 flex items-center gap-2">
                           <CalendarIcon size={14} /> {t('calendar.agenda.empty_today', 'No events scheduled for today')}
                       </div>
                   ) : (
      ...
      {days.length === 0 && (
         <div className="text-center py-10 text-slate-400">
             {t('calendar.agenda.empty_range', 'No dates to display')}
         </div>
      )}
    </div>
  );
