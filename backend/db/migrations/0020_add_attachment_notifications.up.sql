-- Add notification for attachment_uploaded events to support version history
...
BEGIN
  ...
  ELSIF NEW.event_type = 'attachment_uploaded' THEN
    -- File upload events - create notification for each file upload
    node_label := COALESCE(v_node_id, 'узел');
    message := student_name || ' загрузил(а) файл в узле ' || node_label;
