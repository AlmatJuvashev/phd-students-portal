...
func BuildAPI(r *gin.Engine, db *sqlx.DB, cfg config.AppConfig, playbookManager *pb.Manager) *gin.Engine {
    ...
    mon := api.Group("/admin")
    mon.Use(middleware.AuthRequired([]byte(cfg.JWTSecret)))
    mon.Use(middleware.RequireRoles("admin", "superadmin", "advisor"))
    mon.GET("/monitor/students", adminHandler.MonitorStudents)
    mon.GET("/monitor/analytics", adminHandler.MonitorAnalytics)
    mon.GET("/students/:id/journey", adminHandler.StudentJourney)
    mon.PATCH("/students/:id/nodes/:nodeId/state", adminHandler.PatchStudentNodeState)
    mon.GET("/students/:id/deadlines", adminHandler.GetStudentDeadlines)
    mon.PUT("/students/:id/nodes/:nodeId/deadline", adminHandler.PutStudentDeadline)
    mon.POST("/reminders", adminHandler.PostReminders)
