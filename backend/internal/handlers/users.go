...
func (h *UsersHandler) CreateUser(c *gin.Context) {
	...
	if req.Role == "superadmin" {
		c.JSON(http.StatusForbidden, gin.H{"error":"only superadmin can create superadmin"}); return
	}
	...
	for i := 0; i < 1000; i++ {
		...
		if !exists { username = u; break }
	}
	if username == "" {
		c.JSON(http.StatusInternalServerError, gin.H{"error":"could not generate username"}); return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error":"insert failed"}); return
	}
...
func (h *UsersHandler) ChangeOwnPassword(c *gin.Context) {
	...
	if uid == "" { c.JSON(401, gin.H{"error":"missing user id"}); return }
...
func (h *UsersHandler) ResetPasswordForUser(c *gin.Context) {
	...
	if role == "superadmin" {
		c.JSON(403, gin.H{"error":"cannot change superadmin password"}); return
	}
	var req resetPwReq
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return
...
type setActiveReq struct { Active bool `json:"active"` }
