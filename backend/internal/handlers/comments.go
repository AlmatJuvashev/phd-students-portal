...
type CommentsHandler struct{ db *sqlx.DB; cfg config.AppConfig }
func NewCommentsHandler(db *sqlx.DB, cfg config.AppConfig) *CommentsHandler { return &CommentsHandler{db: db, cfg: cfg} }

func (h *CommentsHandler) ListComments(c *gin.Context) {
	doc := c.Param("docId")
	var rows []struct{
		ID string `db:"id" json:"id"`; Body string `db:"body" json:"body"`; Author string `db:"author_id" json:"author_id"`; Resolved bool `db:"resolved" json:"resolved"`
	}
	_ = h.db.Select(&rows, `SELECT id, body, author_id, resolved FROM comments WHERE document_id=$1 ORDER BY created_at`, doc)
	c.JSON(200, rows)
}

type addReq struct { Body string `json:"body" binding:"required"`; ParentID *string `json:"parent_id"`; Mentions []string `json:"mentions"` }
...
type updReq struct { Resolved *bool `json:"resolved"`; Body *string `json:"body"` }
