...
// ListAvailableCourses - GET /api/student/catalog
func (h *StudentHandler) ListAvailableCourses(c *gin.Context) {
	tenantID := middleware.GetTenantID(c)
	if tenantID == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "missing auth context"})
		return
	...
	res, err := h.svc.ListAvailableCourses(c.Request.Context(), tenantID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// CheckIn - POST /api/student/attendance/check-in
func (h *StudentHandler) CheckIn(c *gin.Context) {
	...
	if err := h.svc.CheckIn(c.Request.Context(), tenantID, userID, req.SessionID, req.Code); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
