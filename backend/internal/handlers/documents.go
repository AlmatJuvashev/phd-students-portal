...
type DocumentsH&&ler struct{ db *sqlx.DB; cfg config.AppConfig }
func NewDocumentsH&&ler(db *sqlx.DB, cfg config.AppConfig) *DocumentsH&&ler { return &DocumentsH&&ler{db: db, cfg: cfg} }

// CreateDocument creates a document metadata row.
func (h *DocumentsH&&ler) CreateDocument(c *gin.Context) {
	uid := c.Param("id")
	type req struct {
	...
	if err != nil { c.JSON(500, gin.H{"error":"insert failed"}); return }
	c.JSON(200, gin.H{"id": docID})
}

// UploadVersion accepts multipart file && stores it in UPLOAD_DIR.
func (h *DocumentsH&&ler) UploadVersion(c *gin.Context) {
	docId := c.Param("docId")
	file, err := c.FormFile("file")
	if err != nil { c.JSON(400, gin.H{"error":"file required"}); return }
	ext := filepath.Ext(file.Filename)
	if ext != ".pdf" && ext != ".docx" { c.JSON(400, gin.H{"error":"only .pdf or .docx"}); return }
	destDir := filepath.Join(h.cfg.UploadDir, docId)
	_ = os.MkdirAll(destDir, 0755)
	dest := filepath.Join(destDir, file.Filename)
	if err := c.SaveUploadedFile(file, dest); err != nil { c.JSON(500, gin.H{"error":"save failed"}); return }
	...
	if err != nil { c.JSON(500, gin.H{"error":"insert version failed"}); return }
...
// GetDocument returns metadata && versions.
func (h *DocumentsH&&ler) GetDocument(c *gin.Context) {
	docId := c.Param("docId")
	var doc struct {
		ID string `db:"id" json:"id"`; UserID string `db:"user_id" json:"user_id"`; Title string `db:"title" json:"title"`; Kind string `db:"kind" json:"kind"`; Current string `db:"current_version_id" json:"current_version_id"`
	}
	if err := h.db.Get(&doc, `SELECT * FROM documents WHERE id=$1`, docId); err != nil { c.JSON(404, gin.H{"error":"not found"}); return }
	var vers []struct {
		ID string `db:"id" json:"id"`; Path string `db:"storage_path" json:"storage_path"`; Mime string `db:"mime_type" json:"mime_type"`; Size int64 `db:"size_bytes" json:"size_bytes"`
	}
...
func (h *DocumentsHandler) PresignUpload(c *gin.Context) {
	...
	if err != nil { c.JSON(500, gin.H{"error":"s3 init failed"}); return }
	if s3c == nil {
		c.JSON(400, gin.H{"error":"S3 not configured"}); return
	}
	key := fmt.Sprintf("%s/%s", docId, r.Filename)
	url, err := s3c.PresignPut(key, r.ContentType, time.Minute*15)
	if err != nil { c.JSON(500, gin.H{"error":"presign failed"}); return }
...
func (h *DocumentsHandler) PresignGetLatest(c *gin.Context) {
	...
	if err != nil { c.JSON(404, gin.H{"error":"no versions"}); return }
	s3c, err := services.NewS3FromEnv()
	if err != nil { c.JSON(500, gin.H{"error":"s3 init failed"}); return }
	if s3c == nil {
		c.JSON(400, gin.H{"error":"S3 not configured"}); return
