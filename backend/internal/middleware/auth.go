...
func RequireRoles(roles ...string) gin.HandlerFunc {
	set := map[string]bool{}
	for _, r := range roles { set[r]=true }
	return func(c *gin.Context) {
		val, ok := c.Get("claims")
		if !ok {
...
func HydrateUserFromClaims(c *gin.Context, dbx *sqlx.DB, rds interface{}) {
	...
	// try redis
	var rc *redis.Client
	if v, ok := rds.(*redis.Client); ok { rc = v }
	if rc != nil {
		if s, err := db.CacheGet(rc, "user:"+sub); err == nil && s != "" {
			var u userLite; _ = json.Unmarshal([]byte(s), &u)
			c.Set("current_user", u); return
		}
	}
	var u userLite
	_ = dbx.Get(&u, `SELECT id,username,email,first_name,last_name,role FROM users WHERE id=$1`, sub)
	b, _ := json.Marshal(u)
	if rc != nil { db.CacheSet(rc, "user:"+sub, string(b), time.Minute*10) }
