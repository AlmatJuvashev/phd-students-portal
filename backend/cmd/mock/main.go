...
func createStudent(conn *sqlx.DB, advisorID string) seededUser {
	...
	var id string
	err := conn.Get(&id, `INSERT INTO users (username,email,first_name,last_name,role,password_hash,is_active)
        VALUES ($1,$2,$3,$4,'student',$5,true) RETURNING id`, username, email, first, last, hash)
	if err != nil {
	...
	cohort := randomCohort()
	profileData := fmt.Sprintf(`{"phone":"%s","program":"%s","department":"%s","cohort":"%s"}`, phone, program, department, cohort)
